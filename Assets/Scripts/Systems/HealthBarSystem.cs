    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach((RefRW<LocalTransform> localTransform, RefRO<HealthBar> healthBar) in SystemAPI.Query<RefRW<LocalTransform>, RefRO<HealthBar>>()) 
        {
            ...
            if (localTransform.ValueRO.Scale == 1f) 
                localTransform.ValueRW.Rotation = parentLocalTransform.InverseTransformRotation(Quaternion.LookRotation(cameraForward));
            ...
            postTransformMatrix.ValueRW.Value = float4x4.Scale(healthNormalized, 1, 1);
        }
    }
}
